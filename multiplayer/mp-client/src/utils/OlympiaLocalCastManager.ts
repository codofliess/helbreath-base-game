...
import { ITEM_DROPPED_TO_GROUND } from '../constants/EventNames';
import { EFFECT_ICE_STRIKE_LARGE_SHARD } from '../constants/Effects';
import {
    getGenericSpellVfx,
    hasDedicatedCastHandler,
    isUtilityOnlySpell,
} from '../constants/SpellEffects';
 ...
 */
import { isServerAuthoritativeOlympiaSpell } from '../constants/OlympiaServerSpellMap';
import { IN_UI_CHANGE_MAP } from '../constants/EventNames';

export class OlympiaLocalCastManager {
    private scene: Scene;
    ...
    private executeSpell(data: PlayerConfirmSpellTargetEvent): void {
        ...
        if (isUtilityOnlySpell(data.spellId) && this.executeUtilitySpell(data)) {
            this.castReady = true;
            return;
        }

        if (hasDedicatedCastHandler(data.spellId)) {
            this.executeDedicatedSpell(data, playerPos, onEffectCreated);
            this.castReady = true;
            return;
        }

        if (this.executeGenericSpellVfx(data, playerPos, onEffectCreated)) {
            this.castReady = true;
            return;
        }

        if (this.executeUtilitySpell(data)) {
            this.castReady = true;
            return;
        }

        console.warn(`[CastManager] No handler for spell ${data.spellId}`);
        this.castReady = true;
    }

    private executeDedicatedSpell(
        data: PlayerConfirmSpellTargetEvent,
        ...
        switch (data.spellId) {
            ...
            case SPELL_LIGHTNING_STRIKE_ID:
                ...
                break;
            case SPELL_MASS_LIGHTNING_ARROW_ID:
                new LightningStrike(
                    this.scene,
                    data.originPixelX,
                    data.originPixelY,
    ...
    /** Plays VFX for spells without a dedicated CastManager handler. */
    private executeGenericSpellVfx(
        data: PlayerConfirmSpellTargetEvent,
        playerPos: { x: number; y: number } | undefined,
        onEffectCreated: (effect: Effect) => () => void,
    ): boolean {
        const vfx = getGenericSpellVfx(data.spellId);
        if (!vfx) {
            return false;
        }
        ...
        const targetWorldX = vfx.atCaster
            ? convertPixelPosToWorldPos(data.originPixelX)
            : convertPixelPosToWorldPos(data.targetPixelX);
        const targetWorldY = vfx.atCaster
            ? convertPixelPosToWorldPos(data.originPixelY)
            : convertPixelPosToWorldPos(data.targetPixelY);
        ...
        switch (vfx.kind) {
            case 'energy-bolt':
                new EnergyBolt(
                    ...
                    targetWorldX,
                    targetWorldY,
                    { projectileSpeed: 1500, ...sharedConfig },
                );
                return true;
            case 'lightning-bolt':
                new LightningBolt(
            ...
            case 'effect':
                if (vfx.effectKey) {
                    drawEffect(this.scene, targetWorldX, targetWorldY, vfx.effectKey, {
                        soundManager: this.soundManager,
                        playerWorldX: playerPos?.x,
                        playerWorldY: playerPos?.y,
                        infiniteLoop: vfx.infiniteLoop,
                    });
                }
                return true;
            case 'effects':
                for (const effectKey of vfx.effectKeys ?? []) {
                    drawEffect(this.scene, targetWorldX, targetWorldY, effectKey, {
                        soundManager: this.soundManager,
                        playerWorldX: playerPos?.x,
                        playerWorldY: playerPos?.y,
                    });
    ...
    /** Utility spells (Recall, Create Food, Heal logic). Returns true if handled. */
    private executeUtilitySpell(data: PlayerConfirmSpellTargetEvent): boolean {
        switch (data.spellId) {
            case SPELL_RECALL_ID: {
                EventBus.emit(IN_UI_CHANGE_MAP, 'aresden');
                console.log('[OlympiaLocalCast] Recall → aresden');
                return true;
            }
            case SPELL_CREATE_FOOD_ID: {
                const worldX = convertPixelPosToWorldPos(data.targetPixelX);
                const worldY = convertPixelPosToWorldPos(data.targetPixelY);
                EventBus.emit(ITEM_DROPPED_TO_GROUND, {
                    worldX,
                    worldY,
                    itemId: 171,
                    itemUid: String(Date.now()),
                    quantity: 1,
                });
                console.log(`[CastManager] Create Food at (${worldX}, ${worldY})`);
                return true;
            }
            case SPELL_HEAL_ID:
                this.executeGenericSpellVfx(
                    data,
                    this.getPlayerWorldPos(),
                    this.getOnEffectCreated(),
                );
                return true;
            default:
                return false;
