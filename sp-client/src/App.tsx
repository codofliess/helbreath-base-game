...
import { QuestDialog } from './ui/dialogs/QuestDialog';
...
import { questStore, openQuestDialog } from './ui/store/Quest.store';
import { OUT_UI_OPEN_MAGIC_SHOP, OUT_UI_OPEN_QUEST_DIALOG } from './constants/EventNames';
...
function App()
{
    ...
    const [magicShopDialogPosition, setMagicShopDialogPosition] = useState({ x: 620, y: DIALOG_START_Y });
    const [questDialogPosition, setQuestDialogPosition] = useState({ x: 400, y: DIALOG_START_Y });
    ...
    const [magicShopDialogZIndex, setMagicShopDialogZIndex] = useState(10014);
    const [questDialogZIndex, setQuestDialogZIndex] = useState(10015);
    ...
    const nextZIndexRef = useRef(10016);
    ...
    const showQuestDialog = useStore(questStore, (state) => state.isDialogOpen);
    ...
    useEffect(() => {
        const handleOpenMagicShop = () => setMagicShopOpen(true);
    ...
    useEffect(() => {
        const handleOpenQuest = (data: { personId: number }) => openQuestDialog(data.personId);
        EventBus.on(OUT_UI_OPEN_QUEST_DIALOG, handleOpenQuest);
        return () => {
            EventBus.off(OUT_UI_OPEN_QUEST_DIALOG, handleOpenQuest);
        };
    }, []);
    ...
    useEffect(() => {
        if (showQuestDialog) {
            bringDialogToFront('quest-dialog');
        }
    }, [showQuestDialog]);
    ...
    const bringDialogToFront = (dialogId: string) => {
        ...
        switch (dialogId) {
            ...
            case 'magic-shop-dialog':
                ...
                break;
            case 'quest-dialog':
                setQuestDialogZIndex(currentZIndex);
                break;
    ...
    const handleDragEnd = (event: any) => {
        ...
        switch (dialogId) {
            ...
            case 'cast-dialog':
                ...
                break;
            case 'magic-shop-dialog':
                currentPosition = magicShopDialogPosition;
                break;
            case 'quest-dialog':
                currentPosition = questDialogPosition;
                break;
            case 'inventory-dialog':
        ...
        switch (dialogId) {
            ...
            case 'cast-dialog':
                ...
                break;
            case 'magic-shop-dialog':
                setMagicShopDialogPosition(position);
                break;
            case 'quest-dialog':
                setQuestDialogPosition(position);
                break;
            case 'inventory-dialog':
    ...
    return (
        <DndContext onDragEnd={handleDragEnd}>
            <div id="app">
                ...
                {showQuestDialog && (
                    <QuestDialog
                        position={questDialogPosition}
                        zIndex={questDialogZIndex}
                        onBringToFront={() => bringDialogToFront('quest-dialog')}
                    />
                )}
