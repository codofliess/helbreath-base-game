    ...
    if (src) {
        return (
            <button
                ...
                className="character-sprite-btn"
                style={{
            ...
            >
                <img src={src} alt={title} className="character-sprite-btn-img" draggable={false} />
            </button>
        );
    }
...
function SubPanelHeader({ title, onBack }: { title: string; onBack: () => void }) {
    return (
        <div className="character-subpanel-header">
            <button type="button" className="character-dialog-text-btn" onClick={onBack}>
                ← Volver
            </button>
...
function QuestPanel({ onBack }: { onBack: () => void }) {
    const activeEntries = useStore(questStore, () => getActiveQuestEntries());
            ...
            {activeEntries.length === 0 ? (
                <>
                    ...
                    <p className="character-subpanel-hint">
                        Hablá con Enzu (Farm), Lysio/Lisyo (Garden), o los guardias de ciudad para aceptar misiones.
                    </p>
                </>
            ) : (
                <div className="character-quest-list">
                    {activeEntries.map(({ quest, progress }) => {
                        const ready = isQuestReadyToTurnIn(quest.id);
                        return (
                            <div key={quest.id} className="character-quest-entry">
                                <div className="character-quest-name">
                                    {quest.name}
                                    {ready && <span className="character-quest-ready"> ✓ Listo</span>}
                                </div>
                                <div className="character-quest-progress">
                                    {getQuestProgressLabel(quest, progress)}
                                </div>
                            </div>
                        );
                    })}
...
function LevelSetPanel({ onBack }: { onBack: () => void }) {
    ...
    const draft = useStore(playerDialogStore, (s) => s.levelUpDraft);
            ...
            <div className="character-levelset-grid">
                {rows.map(({ key, label, base }) => (
                    <div key={key} className="character-levelset-row">
                        ...
                        <button type="button" className="character-levelset-btn" disabled={draft.pointsLeft <= 0} onClick={() => adjustLevelUpStat(key, 1)}>+</button>
                        <button type="button" className="character-levelset-btn" disabled={draft[key] <= 0} onClick={() => adjustLevelUpStat(key, -1)}>−</button>
                    </div>
                ))}
            </div>
            <button type="button" className="character-dialog-text-btn" onClick={resetLevelUpDraft} style={{ marginTop: 8 }}>
                Reiniciar
            </button>
        </div>
    );
...
export function PlayerDialog({
    ...
    const spriteFrameMap = useStore(appStore, (s) => s.spriteFrameMap);
    const stats = useStore(playerDialogStore, (s) => s.stats);
    const activeSubPanel = useStore(playerDialogStore, (s) => s.activeSubPanel);
    const guild = useStore(guildStore);
                ...
                {activeSubPanel === 'main' && (
                    <>
                        ...
                        <div className="character-dialog-footer">
                            ...
                            />
                            <button type="button" className="character-dialog-text-btn" onClick={() => openSubPanel('statistics')}>
                                Statistics
                            </button>
                            <button type="button" className="character-dialog-text-btn" onClick={() => openSubPanel('achievements')}>
                                Achievements
                            </button>
                            <button type="button" className="character-dialog-text-btn" onClick={() => openSubPanel('guild')}>
                                Guild
                            </button>
                            <button type="button" className="character-dialog-text-btn" onClick={() => openSubPanel('party')}>
                                Party
                            </button>
                            <SpriteButton
                            ...
                            />
                            <button type="button" className="character-dialog-text-btn" onClick={() => openSubPanel('feedback')}>
                                Feedback
                            </button>
                        </div>
                    </>
