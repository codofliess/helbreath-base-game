...
public static class PlaytestElonQaKit {
    ...
    /// hide it. Never writes <c>Chars/</c> (PauPau / live slots). Keeps Elon's later sandbox edits
    /// (any non-starter save) so GM self-edit is not wiped on restart.
    /// </summary>
    public static void EnsureSeeded(string charsPlaytestDirectory) {
        ...
        if (existing is not null && !IsStarterL1(existing)) {
            Console.WriteLine(
                $"[PLAYTEST] ElonQa sandbox save kept in '{destPath}' (L{existing.Level}, GM edits preserved).");
            return;
        }
    ...
    public static bool ShouldWriteSave(string charsPlaytestDirectory, PlayerPersistenceState incoming) {
        ...
        if (!IsStarterL1(existing) && IsStarterL1(incoming)) {
            Console.WriteLine(
                "[PLAYTEST] Refusing to overwrite the ElonQa sandbox save with a starter L1 snapshot.");
            return false;
        }
