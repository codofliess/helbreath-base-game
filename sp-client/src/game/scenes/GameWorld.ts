...
import { canvasToScreenPosition, convertWorldPosToPixelPos, convertPixelPosToWorldPos, getNextDirection, Direction, findMovableLocation, getDistance, isCellMovable, resolveSafePlayerSpawn } from '../../utils/CoordinateUtils';
...
import { getMusicManager, getGameStateManager, getLootManager, setDebugModeEnabled, setDisplayLargeItemsEnabled, setSoundManager, tryGetMap } from '../../utils/RegistryUtils';
...
import { getMapNpcDisplayName, getMapNpcGreeting, getMapNpcPlacements } from '../../constants/MapNpcPlacements';
import { normalizeMapId } from '../../constants/MapTeleportLocs';
...
import {
    ...
    OUT_UI_OPEN_QUEST_DIALOG,
} from '../../constants/EventNames';
import { onMonsterKilled } from '../../ui/store/Quest.store';
...
export class GameWorld extends Scene {
    ...
    /** Prevents re-triggering warps while a transition is in progress */
    private warpTransitionInProgress = false;
    /** NPC the player is walking toward to interact with */
    private pendingNpcInteraction: NPC | undefined;
    private readonly onChangeMap = (data: {
        targetMap?: string;
        targetX?: number;
        targetY?: number;
        fromTownMap?: string;
        enteringInterior?: boolean;
    }) => {
        if (!data?.targetMap || this.warpTransitionInProgress) {
            return;
        }
        this.warpTransitionInProgress = true;
        this.pendingNpcInteraction = undefined;
        // Block input immediately — gameState map name updates before the new .amd is registered.
        this.loadingMap = true;
        const mapFileName = data.targetMap.endsWith('.amd') ? data.targetMap : `${data.targetMap}.amd`;
        const targetX = data.targetX ?? 50;
        const targetY = data.targetY ?? 50;
        console.log(`🗺️ Warp → ${mapFileName} (${targetX}, ${targetY})`);

        this.player?.cancelMovement();
        getLootManager(this.game).clear();
        const gameStateManager = getGameStateManager(this.game);
        if (data.enteringInterior && data.fromTownMap) {
            gameStateManager.setLastOutdoorMap(data.fromTownMap);
        }
        gameStateManager.setMap(mapFileName);
        gameStateManager.setWorldPos(targetX, targetY);
        gameStateManager.saveGameState();

        // Defer restart — calling scene.restart() synchronously during update() freezes Phaser.
    ...
    private tryPlayerWarp = (tileX?: number, tileY?: number): void => {
        if (!this.player || this.warpTransitionInProgress || this.loadingMap) {
            return;
        }

        const gameStateManager = getGameStateManager(this.game);
        MapWarpSystem.getInstance().checkWarp(
            this.getCurrentMap(),
            gameStateManager.getMap(),
            tileX ?? this.player.getWorldX(),
            tileY ?? this.player.getWorldY(),
            this.player.getIsMoving(),
            gameStateManager.getLastOutdoorMap(),
        );
    };
    ...
    public init() {
        ...
        this.loadingMap = true;
        this.warpTransitionInProgress = false;
    ...
    public create() {
        ...
        EventBus.off('change-map', this.onChangeMap);
        EventBus.on('change-map', this.onChangeMap);
    }
    ...
    public update(_time: number, delta: number): void {
        ...
        if (this.player) {
            ...
            // Warp when idle on a teleport tile (also checked on PLAYER_POSITION_CHANGED)
            if (!this.warpTransitionInProgress && !this.loadingMap) {
                this.tryPlayerWarp();
            }
        }
    ...
    private handleLeftMouseButton(): void {
        ...
        if (!this.loadingMap && inputManager?.getIsLeftMouseDown() && inputManager.getActivePointer() && this.cameras?.main && this.player) {
            ...
            const npc = this.getNpcUnderPointer(pointer);
            const monster = npc ? undefined : this.getMonsterUnderPointer(pointer);
    ...
    private getCurrentMap(): HBMap {
        const mapName = getGameStateManager(this.game).getMap();
        const fromRegistry = tryGetMap(this, mapName);
        if (fromRegistry) {
            return fromRegistry;
        }
    ...
    private getNpcUnderPointer(pointer: Phaser.Input.Pointer): NPC | undefined {
        if (!this.cameras?.main) {
            return undefined;
        }
        const camera = this.cameras.main;
        ...
        const candidates = getObjectsNearPixel(
            this.npcs.filter((npc) => !npc.getIsDead()),
            (npc) => ({ x: npc.getAnimatedPixelX(), y: npc.getAnimatedPixelY() }),
            worldPixelX,
            worldPixelY,
            10,
        );
    ...
    private interactWithNpc(npc: NPC): void {
        ...
        if (npc.getRole() === 'magic-shop') {
            EventBus.emit(OUT_UI_OPEN_MAGIC_SHOP);
        } else if (npc.getRole() === 'quest-giver' && npc.getPersonId() !== undefined) {
            EventBus.emit(OUT_UI_OPEN_QUEST_DIALOG, { personId: npc.getPersonId() });
        }

        new FloatingText(this, {
            text: npc.getGreeting(),
    ...
    private tryInteractWithNpc(npc: NPC): boolean {
        ...
        this.player.setDestination(npc.getWorldX(), npc.getWorldY(), false);
        return true;
    }

    private tryCompletePendingNpcInteraction(): void {
        const npc = this.pendingNpcInteraction;
        if (!npc || !this.player || this.player.getIsMoving() || npc.getIsDead()) {
            return;
        }
    ...
    private setupPlayerEventListeners(): void {
        // Listen for player position changes (spatial audio + warp when landing on teleport tiles)
    ...
    private setupMonsterEventListeners(): void {
        ...
        EventBus.on(MONSTER_DEAD, (data: { monsterId: number }) => {
            ...
            if (monsterIndex !== -1) {
                ...
                const mapName = getGameStateManager(this.game).getMap();
                onMonsterKilled(monster.getSpriteName(), mapName);
    ...
    private setupInputManager(): void {
        this.inputManager = new InputManager({
            scene: this,
            isEnabled: () => !this.loadingMap && !this.warpTransitionInProgress,
            onPointerMove: (worldPixelX, worldPixelY) => {
                if (this.loadingMap || this.warpTransitionInProgress) {
                    return;
                }
     ...
     */
    private initializeGameObjects(map: HBMap): void {
        ...
        const isWithinMapBounds = (x: number, y: number): boolean =>
            x >= 0 && y >= 0 && x < map.sizeX && y < map.sizeY;
        ...
        } else if (!isWithinMapBounds(savedWorldPosX, savedWorldPosY)) {
            playerWorldX = Math.floor(map.sizeX / 2);
            playerWorldY = Math.floor(map.sizeY / 2);
            console.warn(
                '[GameWorld] Saved coordinates out of map bounds, using center:',
                { savedWorldPosX, savedWorldPosY, mapSizeX: map.sizeX, mapSizeY: map.sizeY, playerWorldX, playerWorldY },
            );
        } else {
        ...
        const currentMapId = normalizeMapId(gameStateManager.getMap());
        const beforeX = playerWorldX;
        const beforeY = playerWorldY;
        const safeSpawn = resolveSafePlayerSpawn(map, currentMapId, playerWorldX, playerWorldY);
        playerWorldX = safeSpawn.x;
        playerWorldY = safeSpawn.y;
        if (playerWorldX !== beforeX || playerWorldY !== beforeY) {
            console.log('[GameWorld] Relocated off warp tile:', {
                from: { x: beforeX, y: beforeY },
                to: { x: playerWorldX, y: playerWorldY },
                mapId: currentMapId,
            });
        ...
        void this.spawnMapNpcs(map);
    }

    /**
     * Spawns stationary NPCs configured for the current interior map (shop, blacksmith, guild hall, etc.).
     */
    private async spawnMapNpcs(map: HBMap): Promise<void> {
        const mapName = getGameStateManager(this.game).getMap();
        const placements = getMapNpcPlacements(mapName);
        ...
        for (const placement of placements) {
            const spriteName = placement.sprite.toLowerCase();
            const npcData = getNPCData(spriteName);
            if (!npcData && !placement.displayName) {
                console.warn(`[GameWorld] Skipping map NPC: unknown sprite ${placement.sprite}`);
                continue;
            }

            try {
                if (!areMonsterAssetsLoaded(this, spriteName)) {
                    await loadMonsterAssetsOnDemand(this, spriteName);
                }

                const npcId = this.nextNPCId++;
                const displayName = getMapNpcDisplayName(spriteName, placement.displayName);
                const npc = new NPC(this, {
                    x: placement.x,
                    y: placement.y,
                    spriteName,
                    displayName,
                    greeting: placement.greeting ?? getMapNpcGreeting(mapName, spriteName, displayName, placement.greeting),
                ...
                this.npcs.push(npc);
                console.log(
                    `[GameWorld] Spawned map NPC ${npcData.name} at (${placement.x}, ${placement.y}) on ${mapName}`,
                );
            } catch (error) {
                console.error(`[GameWorld] Failed to spawn map NPC ${spriteName} on ${mapName}:`, error);
            }
        }
    }
    ...
    private setupMap(map: HBMap): void {
        ...
        this.initializeGameObjects(map);
