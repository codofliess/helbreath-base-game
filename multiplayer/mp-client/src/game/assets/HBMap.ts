...
export class HBMapTile {
    ...
    constructor(
        ...
        this.objectSpriteFrame = view.getInt16(offset + 6, true);
...
export class HBMap {
    ...
    /** Server-authored teleport trigger cells (from InitialGameWorldState.teleportLocs), not .amd tile flags */
    private serverTeleportSourceCells: { x: number; y: number }[] = [];

    /** Graphics for server teleport cell highlights */
    private serverTeleportCellGraphics: Phaser.GameObjects.Graphics[] = [];

    /** Whether server teleport highlighting is enabled */
    private isServerTeleportCellsHighlightEnabled = false;

    /** Scene used while server teleport highlights are active (for redraw when source list updates) */
    private serverTeleportHighlightScene: Scene | undefined = undefined;
    ...
    private paintStreamedGround(scene: Phaser.Scene, rect: MapTileRect): void {
        ...
        tileKeys.forEach((tileKey, index) => {
            ...
            const canDraw =
                ...
                (typeof ImageBitmap !== 'undefined' && sourceImage instanceof ImageBitmap);
            if (!canDraw) {
    ...
    /**
     * Returns the first streamed row tilemap layer (legacy accessor).
     */
    ...
    public destroyAllHighlights(): void {
        ...
        this.disableServerTeleportCellsHighlight();
    ...
    /**
     * Stores server teleport source cells; redraws the overlay if server teleport highlight is on.
     */
    public setServerTeleportSourceCells(cells: ReadonlyArray<{ x: number; y: number }>): void {
        this.serverTeleportSourceCells = cells.map((c) => ({ x: c.x, y: c.y }));
        if (this.isServerTeleportCellsHighlightEnabled && this.serverTeleportHighlightScene) {
            this.redrawServerTeleportCellsHighlight(this.serverTeleportHighlightScene);
        }
    }

    /**
     * Highlights server teleport cells (darker blue than map .amd teleport flags).
     */
    public enableServerTeleportCellsHighlight(scene: Scene): void {
        if (!this.loaded) {
            throw new Error('Map must be loaded before enabling server teleport cell highlighting');
        }

        if (this.isServerTeleportCellsHighlightEnabled) {
            return;
        }

        this.serverTeleportHighlightScene = scene;
        this.redrawServerTeleportCellsHighlight(scene);
        this.isServerTeleportCellsHighlightEnabled = true;
        console.log(`Server teleport cell highlighting enabled: ${this.serverTeleportCellGraphics.length} cells`);
    }

    public disableServerTeleportCellsHighlight(): void {
        if (!this.isServerTeleportCellsHighlightEnabled) {
            return;
        }

        this.serverTeleportCellGraphics.forEach((graphics) => {
            graphics.destroy();
        });
        this.serverTeleportCellGraphics = [];
        this.isServerTeleportCellsHighlightEnabled = false;
        this.serverTeleportHighlightScene = undefined;

        console.log('Server teleport cell highlighting disabled');
    }

    private redrawServerTeleportCellsHighlight(scene: Scene): void {
        this.serverTeleportCellGraphics.forEach((graphics) => {
            graphics.destroy();
        });
        this.serverTeleportCellGraphics = [];

        for (const { x, y } of this.serverTeleportSourceCells) {
            ...
            graphics.fillRect(worldX, worldY, TILE_SIZE, TILE_SIZE);
            graphics.setDepth(2);
            this.serverTeleportCellGraphics.push(graphics);
        }
    }
