...
const { store: inventoryDialogStore, toggle: toggleInventoryDialog, setOpen: setInventoryDialogOpen } =
...
EventBus.on(ITEM_REMOVED_FROM_BAG, (payload: { itemUid: string }) => {
...
EventBus.on(
    ITEM_BAG_POSITION_UPDATED,
    (payload: { itemUid: string; bagX: number; bagY: number }) => {
        inventoryDialogStore.setState((state) => ({
            ...state,
...
EventBus.on(ITEM_QUANTITY_UPDATED, (payload: { itemUid: string; quantity: number }) => {
    inventoryDialogStore.setState((state) => ({
        ...state,
        baggedItems: state.baggedItems.map((b) =>
...
EventBus.on(ITEM_BAG_ITEM_BROUGHT_TO_FRONT, (payload: { itemUid: string }) => {
    inventoryDialogStore.setState((state) => {
