...
interface GameStateStorage {
    ...
    map: string;
    /** Last outdoor town map (for shared interior building exits) */
...
export class GameStateManager {
    ...
    /** Last outdoor town visited (aresden / elvine) for interior exits */
    private lastOutdoorMap = 'aresden';
    /** Movement speed slider value (0-100) */
    ...
    constructor() {
        ...
        try {
            ...
            if (gameStateJson) {
                ...
                if (typeof gameState.lastOutdoorMap === 'string' && gameState.lastOutdoorMap.length > 0) {
                    this.lastOutdoorMap = gameState.lastOutdoorMap;
                }
    ...
    public saveGameState(): void {
        try {
            const gameState: GameStateStorage = {
                ...
                map: this.map,
                lastOutdoorMap: this.lastOutdoorMap,
    ...
    public getLastOutdoorMap(): string {
        return this.lastOutdoorMap;
    }

    public setLastOutdoorMap(mapId: string): void {
        this.lastOutdoorMap = mapId;
    }
