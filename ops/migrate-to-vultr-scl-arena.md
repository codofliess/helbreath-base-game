# Hybrid LatAm arena shard: keep CX53 world + add Vultr Santiago (SCL) arena

**Status:** runbook only. This file does **not** provision, spend, or cut over. Do **not** destroy, power off, or DNS-steal live EU (`OLD_IP`).

**Working plan (Elon region cut + Martín hybrid):** keep Hetzner CX53 fsn1 as **world**; add Vultr **Santiago (`scl`)** instance **`cl-arena-scl1`**, plan **`vc2-4c-8gb`** (~**US$40/mo**), **no backups**, same SSH key as fsn1, as **arena-only**. Incremental cost ~**$40**. **São Paulo (`sao`) is discarded** (region premium). **Miami only if SCL has no stock.** Do **not** treat “replace CX53 with a 8/32 world box” as the default (deferred / appendix, pending one-world confirm).
...
| **Arena (add)** | Vultr **Cloud Compute** · region **Santiago (`scl`)** · hostname **`cl-arena-scl1`** · plan **`vc2-4c-8gb`** (4 vCPU / 8 GB, ~**US$40/mo**) · **automatic backups off** · IPv4 **`ARENA_IP`** (placeholder — fill after create) · HvH / BvB / ping-sensitive LatAm |
...
| Hard rule | **No** `hcloud server delete` / no [`ops/ip-swap-to-cx53.sh`](./ip-swap-to-cx53.sh). **Do not** point `play.chainlords.net` A at SCL (or Miami). Rollback = **shut SCL arenas**, world unchanged. |
...
| `ARENA_IP` | Vultr Santiago (`cl-arena-scl1`) public IPv4 — fill **after** provision; never guess |
...
```
  Client
    ...
    └─ arena WS/API  →  ARENA_HOST           →  Vultr scl / cl-arena-scl1  (fightzone* / colosseum / HvH BvB)
...
| Postgres | Railway plugin and/or the **one** canonical URL already on live `chainlords-game` | **Single SoT.** SCL must **not** get its own `helbreath-postgres` with a copied dump as a second character DB. |

**Handoff (required product rule):** enter arena with the **same `playerId`**. Loadout/bag is the **one** persistence row (Postgres), not a second `Chars/*.json` tree on SCL. Arena kits that are already sealed vs world bag (see social arena-kit docs) stay a **mode** on that player, not a second account.
...
## Why this is not CX43→CX53 and not “move play to Chile/Brazil”
...
- `pg_dump` live into a **new** local Docker on SCL (split-brain inventories).
- Change `play.chainlords.net` A → `ARENA_IP`.
- Enable Hetzner autoscaler / upgrade-hunter on Vultr.

Do:

- Copy **binaries + Config** (and nginx pattern) to SCL.
- Point SCL at the **same** `WALLET_AUTH_SECRET` and **same** `DATABASE_URL` as world + middleware.
...
|---|---|---|
| **Vultr `scl` `vc2-4c-8gb` (`cl-arena-scl1`)** | **~US$40/mo** | **Working default.** Arena-only. **No backups.** Same SSH key as fsn1. |
| Hetzner CX53 fsn1 ~8 vCPU / 32 GB | existing | **Keep.** World / cities / heavy maps. |
| Vultr **Miami (`mia`)** same `vc2-4c-8gb` | ~US$40 | **Fallback only** if Santiago has **no stock**. Not a second region in parallel. |
| Vultr **São Paulo (`sao`)** any plan | — | **Discarded** (region premium). Do not create. |
| Vultr **São Paulo (`sao`)** any plan | — | **Discarded** (region premium). Do not create. |

São Paulo (`sao`) was dropped because of **region premium**. Hybrid parks world load on CX53; SCL is arena concurrency, not 150 city-sim. Do not create automatic backups on `cl-arena-scl1`.
...
| JSON `Chars/` on disk | `/opt/chainlords/server/Chars` on fsn1 | **World continues here.** SCL must not become a second bag SoT. Prefer Postgres for cross-shard character state. |
| `Config/` | fsn1 | Copy to SCL; SCL config **restricts maps to arena allowlist** |
| Docker `helbreath-postgres` on VPS | Documented on fsn1 (2026-07-26) | **Stay on fsn1 if still canonical.** Do **not** clone it to SCL. If live `DATABASE_URL` is already Railway public TCP, **both** world and arena use that URL. |
...
0. **Confirm SoT:** live `DATABASE_URL` is the shared Postgres both shards will use. If only local Docker on fsn1 exists, **do not** GO until Railway public URL (or equivalent reachable SoT) is on **both** units — SCL cannot safely own a copied local DB.
...
2. **Provision** Vultr **`scl`** **`vc2-4c-8gb`**, hostname **`cl-arena-scl1`**, **backups off**, **same SSH key** as fsn1 (operator later; this PR does not spend). If `scl` is out of stock, **Miami (`mia`)** same plan — not `sao`. Record **`ARENA_IP`**.
3. **Bootstrap** dest: packages, Docker (optional; not for a second Postgres), nginx, ufw. **No** Hetzner CLI. **No** public arena traffic yet.
4. **Rsync binaries + Config** (and a **staging** client if you serve traveler from `ARENA_HOST`). **Do not** `--delete` live `Chars/` onto SCL as SoT. **Do not** stop fsn1 game.
5. **Install** systemd `chainlords-game` on SCL; **disable** Hetzner hunters. Same secrets. nginx `server_name` = **`ARENA_HOST` + `ARENA_IP`** (not `play.chainlords.net` as the world vhost).
...
9. **Rollback:** stop/disable SCL `chainlords-game` (and/or remove `ARENA_HOST` DNS). Clients fall back to world-only. **Do not** touch CX53 except to confirm it never stopped.
...
|---|---|
| `DATABASE_URL` is Railway **public** TCP | **Good.** Copy the **same** URL to the SCL arena unit. Do not `pg_dump` Railway into SCL Docker. |
...
| Copied local Docker on SCL | **Forbidden** as character SoT — that is two inventories. |
...
## 1. Provision Vultr Santiago arena box `cl-arena-scl1` (operator — not this PR)

Create **after** SoT + endpoint work is agreed. Record values; do not commit secrets or invent IPs.

- [ ] Region: **Santiago (`scl`)** — **not** São Paulo (`sao`, discarded: region premium). **Miami (`mia`) only if `scl` has no stock.**
- [ ] Plan: **`vc2-4c-8gb`** (Cloud Compute Regular 4 vCPU / 8 GB, ~**US$40/mo**) — **arena-only**
- [ ] **Automatic backups: off** (no Vultr backups product on this VM)
- [ ] OS: Ubuntu LTS matching world (`lsb_release -a` on `OLD_IP`)
- [ ] **SSH keys** at create — **same pubkey** as live fsn1 (`hetzner_chainlords` / current `authorized_keys`)
- [ ] Instance hostname / label: **`cl-arena-scl1`** (required)
- [ ] Firewall: see §5
- [ ] Write down **`ARENA_IP`**. Confirm: `ssh -i "$SSH_KEY" "root@${ARENA_IP}" hostname` (expect **`cl-arena-scl1`** if region is `scl`)
...
Same packages as [`ops/migrate-to-cx53.sh`](./migrate-to-cx53.sh) §1, **minus** Hetzner CLI. **No** requirement to run Postgres Docker on SCL.
...
Copy **server binary + Config + ops + nginx**. Do **not** mirror live `Chars/` as the SCL SoT (`--exclude 'server/Chars/'` unless you need empty dir scaffolding).
...
## 4. nginx + secrets on SCL (arena vhost)
...
Do **not** expose Postgres on SCL (there should be no public `:5432`). Prefer game bound `127.0.0.1:1337` behind nginx `/ws` and `/api/`. Mirror ports on the Vultr firewall group.
...
Point **arena** WS/API at `ARENA_HOST` (new env or connect field). Keep **world** on `play.chainlords.net`. Same wallet token (`WALLET_AUTH_SECRET`). Confirm **one** bag after arena match (Postgres), not an SCL-only JSON file.
...
| **Two inventories** | SCL `Chars/` or a cloned Postgres | Shared `DATABASE_URL`; exclude live `Chars/` rsync as SoT; no `pg_dump` clone |
| **nginx** | `/api/` falls through to SPA; or `play.chainlords.net` accidentally served from SCL | Arena vhost = `ARENA_HOST` only; `location /api/` and `/ws` → `127.0.0.1:1337` |
| **Split-brain world** | Both processes simulate cities | SCL **map allowlist** — reject Aresden/Elvine/farms |
| **Hetzner hunters on SCL** | `hcloud` create/destroy EU | Disable autoscaler / upgrade-probe / hunter timers |
...
## 8. Rollback = shut SCL arenas (world stays)
...
4. Leave the Vultr VM in place or destroy later — **optional**; rollback does **not** require destroying SCL, only shutting the arena process/DNS.
...
- [ ] Incremental bill ~**$40** (`cl-arena-scl1` `vc2-4c-8gb`, no backups), CX53 unchanged.
...
|---|---|
| [`ops/deploy-finish.sh`](./deploy-finish.sh) etc. | After GO, run **per host** (world **and** arena) when shipping game bits. `psql` to `127.0.0.1` on SCL will fail if SoT is Railway — use `DATABASE_URL`. |
...
Suggested header if you later add a **non-destructive** `ops/add-vultr-arena-scl.sh`:
...
- [ ] Shared `DATABASE_URL` reachable from SCL (not a cloned local PG)
- [ ] Same `WALLET_AUTH_SECRET` on world, arena, middleware
- [ ] Client has **arena endpoint** vs **world endpoint**
- [ ] SCL map allowlist = arena only (no cities)
- [ ] Handoff = same `playerId` / one inventory (no SCL `Chars/` SoT)
- [ ] `ARENA_IP` recorded; SSH works; hostname **`cl-arena-scl1`**; plan **`vc2-4c-8gb`**; backups **off**; region **`scl`** (or `mia` only if no SCL stock)
- [ ] nginx `ARENA_HOST` + `/api/` + `/ws` → `:1337`
- [ ] `play.chainlords.net` still `46.224.129.38`
- [ ] Hetzner autoscale timers **off** on SCL
- [ ] fsn1 **not** stopped, **not** scheduled for delete
- [ ] Smoke arena `ARENA_HOST` + smoke world `play.chainlords.net`
- [ ] Rollback = **shut SCL arenas** (stop dest game / drop `ARENA_HOST`); world stays
...
- SKU discussion from earlier drafts (São Paulo 8/32 world host) is **not** this GO. If one-world is confirmed later, pick region **without** `sao` premium unless ops revisits; hybrid default remains **`scl` `vc2-4c-8gb`**.
- **Until that confirm:** do **not** size or DNS as if CX53 were being replaced. Hybrid + **$40** SCL arena box is the plan. **Do not** reopen São Paulo (premium).
