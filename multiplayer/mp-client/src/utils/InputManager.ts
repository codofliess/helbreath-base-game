...
export interface InputManagerConfig {
    ...
    isEnabled?: () => boolean;
    /** When false, left-button down/up are ignored (e.g. while map loading overlay is visible). Right button and pointer move still follow `isEnabled`. */
    acceptLeftMouseDown?: () => boolean;
...
export class InputManager {
    ...
    private acceptLeftMouseDown: () => boolean;
    ...
    constructor(config: InputManagerConfig) {
        ...
        this.acceptLeftMouseDown = config.acceptLeftMouseDown ?? (() => true);
    ...
    private handlePointerDown(pointer: Phaser.Input.Pointer): void {
        ...
        if (pointer.leftButtonDown() && !this.acceptLeftMouseDown()) {
            return;
        }
        ...
        try {
            ...
            console.error('[InputManager:pointerDown]', error);
        }
    }
    ...
    private handlePointerUp(pointer: Phaser.Input.Pointer): void {
        ...
        try {
            ...
            if (releasedButton === 0 && this.leftMouseDown) {
                this.leftMouseDown = false;
                this.onPointerUp?.(pointer);
            }

            if (releasedButton === 2 && this.rightMouseDown) {
                this.rightMouseDown = false;
            }
        } catch (error) {
            console.error('[InputManager:pointerUp]', error);
        }
    }
    ...
    private handlePointerOut(): void {
        try {
            ...
            console.error('[InputManager:pointerOut]', error);
        }
    }
}
