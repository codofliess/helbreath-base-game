...
export function getGameStateManager(game: Game): GameStateManager {
    ...
    // Vite HMR can leave a stale registry instance after GameStateManager gains new methods.
    if (typeof gameStateManager.getLastOutdoorMap !== 'function') {
        const refreshed = new GameStateManager();
        game.registry.set(GAME_STATE_MANAGER_KEY, refreshed);
        return refreshed;
    }
 ...
 */
export function getMapRegistryKey(mapName: string): string {
    return `map-${mapName.replace('.amd', '')}`;
}

export function tryGetMap(scene: Scene, mapName: string): HBMap | undefined {
    const mapKey = getMapRegistryKey(mapName);
    return getRegistryValue<HBMap>(scene.registry, mapKey);
}

export function getMap(scene: Scene, mapName: string): HBMap {
    const map = tryGetMap(scene, mapName);

    if (!map) {
        throw Error(`Map not found in registry: ${getMapRegistryKey(mapName)}`);
    }
