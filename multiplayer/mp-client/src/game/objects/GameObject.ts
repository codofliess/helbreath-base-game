...
/**
 * Base entity for players, monsters, and NPCs: grid position, movement interpolation, targeting,
 * combat HP/damage, occupancy on `HBMap`, and one or more `GameAsset` sprites.
 */
export abstract class GameObject {
    ...
    /** Per-tile step duration in ms (align with proto `movementSpeedMs`). Set via constructor config. */
    protected movementSpeedMs: number;

    /**
     * Duration (ms) for the current grid-step interpolation and move animation. Snapshotted at the start of
     * each `move()` from `movementSpeedMs`; mid-step server speed updates only change `movementSpeedMs`.
     */
    protected activeStepDurationMs: number;
    ...
    /** Server-synced temporary effect type ids (align with proto `TemporaryEffectType` numeric values). */
    protected activeTemporaryEffects = new Set<number>();
    ...
    /** Duration (ms) for knockback visual interpolation; defaults to {@link KNOCKBACK_DURATION_MS}. */
    protected knockbackVisualDurationMs: number = KNOCKBACK_DURATION_MS;
    ...
    constructor(scene: Scene, config: GameObjectConfig) {
        ...
        this.movementSpeedMs = config.movementSpeedMs;
        this.activeStepDurationMs = config.movementSpeedMs;

        // Mark initial spawn location as occupied
        this.markCurrentTileOccupied();
    }

    public syncActiveTemporaryEffects(types: readonly number[]): void {
        this.activeTemporaryEffects.clear();
        for (const t of types) {
            this.activeTemporaryEffects.add(t);
        }
        this.onTemporaryEffectsChanged();
    }

    public applyTemporaryEffect(effectType: number): void {
        this.activeTemporaryEffects.add(effectType);
        this.onTemporaryEffectsChanged();
    }

    public removeTemporaryEffect(effectType: number): void {
        this.activeTemporaryEffects.delete(effectType);
        this.onTemporaryEffectsChanged();
    }
    ...
    public hasTemporaryEffect(effectType: number): boolean {
        return this.activeTemporaryEffects.has(effectType);
    }

    /** Subclasses update visuals when server-driven temporary effects change. */
    protected onTemporaryEffectsChanged(): void {}
    ...
    /**
     * Animated pixel X including step offset; horizontal centering uses `worldCellCenterPixelX` (half a tile).
     */
    public getAnimatedPixelX(): number {
        return worldCellCenterPixelX(this.worldX) + this.offsetX;
    }
     ...
     * Centers the object in the cell by adding half a tile from the cell origin.
     *
     * @returns The animated pixel Y coordinate
     */
    public getAnimatedPixelY(): number {
        return worldCellCenterPixelY(this.worldY) + this.offsetY;
    }
     ...
     * @param durationMs - Optional interpolation duration (defaults to {@link KNOCKBACK_DURATION_MS})
     * @param knockbackFromX - When set with knockbackFromY, authoritative pre-knockback cell (server) for slide; current logical cell is still freed as the predicted position.
     * @param knockbackFromY - Paired with {@link knockbackFromX}.
     */
    protected applyKnockbackMovement(
        destX: number,
        destY: number,
        durationMs?: number,
        ...
        const preWorldX = this.worldX;
        const preWorldY = this.worldY;
        const knockbackSourceX =
            knockbackFromX !== undefined && knockbackFromY !== undefined ? knockbackFromX : preWorldX;
        const knockbackSourceY =
            knockbackFromX !== undefined && knockbackFromY !== undefined ? knockbackFromY : preWorldY;

        this.knockbackVisualDurationMs = durationMs ?? KNOCKBACK_DURATION_MS;
        ...
        this.knockbackStartWorldX = knockbackSourceX;
        this.knockbackStartWorldY = knockbackSourceY;
    ...
    protected updateKnockbackVisual(delta: number): boolean {
        ...
        const progress = Math.min(1, this.knockbackElapsedMs / this.knockbackVisualDurationMs);
        ...
        if (progress >= 1) {
            ...
            this.knockbackVisualDurationMs = KNOCKBACK_DURATION_MS;
            this.offsetX = 0;
            this.offsetY = 0;
            this.updatePixelPosition();
    ...
    protected move(direction: Direction): void {
        ...
        this.activeStepDurationMs = this.movementSpeedMs;
    ...
    protected findNextMovableDirection(direction: Direction): Direction {
        ...
        for (let i = startDir; this.playerTurn === 0 ? i <= endDir : i >= endDir; i += step) {
            ...
            const candidateDir = toDirection(tmpDir);
            const [dirX, dirY] = getDirectionOffset(candidateDir);
    ...
    public update(delta: number): void {
        ...
        const progress = Math.min(this.movementElapsedTime / this.activeStepDurationMs, 1.0);
...
/** Configuration for creating a GameObject instance (constructor arg). */
type GameObjectConfig = {
    ...
    /** Per-tile step duration in ms (align with proto `movementSpeedMs`). Use 0 when immobile (e.g. NPC). */
