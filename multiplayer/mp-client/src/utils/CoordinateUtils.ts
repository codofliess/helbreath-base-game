 ...
 * Uses the canvas DOM bounds so the result stays accurate even when CSS scaling
 * or fullscreen transforms change the visible canvas size.
 *
...
export function canvasToScreenPosition(
    ...
    return {
        ...
        screenY: rect.top + (canvasY / baseHeight) * rect.height,
    };
}
...
/** Pixels from cell top-left to cell center (half of {@link TILE_SIZE}). */
export const CELL_CENTER_PIXEL_OFFSET = TILE_SIZE / 2;

/**
 * Pixel X of the center of a world grid cell (top-left of cell + half tile).
 * Matches anchoring used for entities centered in a cell.
 */
export function worldCellCenterPixelX(worldX: number): number {
    return convertWorldPosToPixelPos(worldX) + CELL_CENTER_PIXEL_OFFSET;
}

/**
 * Pixel Y of the center of a world grid cell (top-left of cell + half tile).
 */
export function worldCellCenterPixelY(worldY: number): number {
    return convertWorldPosToPixelPos(worldY) + CELL_CENTER_PIXEL_OFFSET;
}
