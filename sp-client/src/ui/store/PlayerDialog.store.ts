...
export type CharacterSubPanel = 'main' | 'quest' | 'party' | 'levelSet' | 'statistics' | 'achievements' | 'feedback' | 'guild';
...
const initialStats: CharacterStats = {
    ...
    mag: 32,
    chr: 61,
    faction: 'Aresden Soldier',
    talents: 'Long Sword, Xelma',
};
...
export const setPlayerDialogOpen = (value: boolean) => {
    playerDialogStore.setState((state) => ({
        ...state,
        isOpen: value,
        ...(value ? {} : { activeSubPanel: 'main' as CharacterSubPanel }),
    }));
};

export const setCharacterSubPanel = (panel: CharacterSubPanel) => {
    playerDialogStore.setState((state) => ({ ...state, activeSubPanel: panel }));
};
...
export const resetLevelUpDraft = () => {
    playerDialogStore.setState((state) => ({ ...state, levelUpDraft: { ...initialLevelUpDraft } }));
};
