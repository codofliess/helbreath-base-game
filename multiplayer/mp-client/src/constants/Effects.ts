...
/**
 * World VFX: sprite animation at a pixel, optional SFX, then destroy.
 * Keys line up with `EFFECT_*` constants below.
 */
export interface EffectConfig {
    name: string;
    key: string;
    /** Sprite basename (no extension), e.g. `effect11`. */
    sprite: string;
    spriteSheetIndex: number;
    frameRate?: number;
    /** Inclusive frame range; omit to play whole sheet. */
    animationFrames?: [number, number];
    /** SFX on spawn (e.g. `E5.mp3`); spatial mix, no duration stretch. */
    sound?: string;
    /** Added to Y-based depth (default +80); negative draws behind at same Y. */
    depthOffset?: number;
    /** From this frame onward, alpha ramps to 0 over remaining frames. */
    fadeOutStartAnimationIndex?: number;
    /** Draws light-radius sheet behind the main effect. */
...
/** Lookup in `EFFECTS` by `EffectConfig.key`. */
